...
//按删除订单返回问诊记录，请求已提交，不用emit，问诊记录刷新就行了
const {deleteConsultOrder,deleteLoading} = UseDeleteConsultOrder(()=>{router.push('/user/consult')})
//取消订单函数
const {cancelOrder} = useCancelOrder() //不使用loading
const {onShowPrescription} = useShowPrescription()

const route = useRoute()
const item = ref<ConsultOrderItem>()
onMounted(async () => {
  const res = await getConsultOrderDetailAPI(route.params.id as string)
  item.value = res.data
  console.log(item.value.orderNo)
})
...
<template>
  <div class="consult-detail-page" v-if="item">
    <cp-nav-bar :arrow="true" title="问诊详情"/>
    ...
    <div class="detail-order">
      ...
      <van-cell-group :border="false">
        <van-cell title="订单编号">
          <template #value>
            <span class="copy" @click="copyText(<string>route.params.id)">复制</span>
            {{ route.params.id }}
          </template>
        </van-cell>
    ...
    <div class="detail-action van-hairline--top" v-if="item.status === OrderType.ConsultPay">
      ...
      <van-button type="default" round @click="cancelOrder(item)">取消问诊</van-button>
    ...
    <div class="detail-action van-hairline--top" v-if="item.status === OrderType.ConsultChat">
      <van-button type="default" round v-if="item.prescriptionId" @click="onShowPrescription(item?.prescriptionId)">查看处方</van-button>
      <van-button type="primary" round :to="`/room?orderId=${item.id}`">继续沟通</van-button>
    </div>
