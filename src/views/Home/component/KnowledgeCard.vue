...
import {useFollow} from "@/composables/followDoctor";

const { loading,follow } = useFollow()


const props = defineProps<{item:Knowledge}>()
...
<template>
  <div class="knowledge-card van-hairline--bottom">
    <div class="head">
      ...
      <van-button type="primary" :plain='item.likeFlag === 1' :loading="loading" @click="follow(item,'doc')" class="btn" size="small" round>{{ item.likeFlag === 1 ? '已关注' : '+关注' }}</van-button>
    </div>
    <div class="body">
      <h3 class="title van-ellipsis">{{ item.title }}</h3>
      <p class="tag">
        <!--<span v-for="(tag,index) in item.topic" :key="index"># {{ tag }}</span>-->
      ...
      <div class="imgs" :class="{ large: item.coverUrl.length === 1 }">
        <van-image
            fit="cover"
            v-for="(url,i) in item.coverUrl" :key='i'
            :src="url"
        />
      </div>
      <p class="logs">
